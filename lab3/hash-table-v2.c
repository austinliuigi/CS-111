...
struct hash_table_v2 *hash_table_v2_create() {
    ...
    for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
        ...
        mutex_err = pthread_mutex_init(&entry->entry_mutex, NULL);
        if (mutex_err)
            exit(mutex_err);
...
void hash_table_v2_add_entry(struct hash_table_v2 *hash_table, const char *key,
    ...
    int mutex_err;
    mutex_err = pthread_mutex_lock(&hash_table_entry->entry_mutex);
    if (mutex_err)
        exit(mutex_err);
    struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
    ...
    mutex_err = pthread_mutex_unlock(&hash_table_entry->entry_mutex);
    if (mutex_err)
        exit(mutex_err);
}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table) {
    ...
    for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
        struct hash_table_entry *entry = &hash_table->entries[i];
        mutex_err = pthread_mutex_destroy(&entry->entry_mutex);
        if (mutex_err)
            exit(mutex_err);
