...
struct hash_table_v1 *hash_table_v1_create() {
    ...
    int mutex_err = pthread_mutex_init(&hash_table->table_mutex, NULL);
    if (mutex_err)
        exit(mutex_err);
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table, const char *key,
                             uint32_t value) {
    int mutex_err;
    mutex_err = pthread_mutex_lock(&hash_table->table_mutex);
    if (mutex_err)
        exit(mutex_err);
    struct hash_table_entry *hash_table_entry =
        get_hash_table_entry(hash_table, key);
    ...
    mutex_err = pthread_mutex_unlock(&hash_table->table_mutex);
    if (mutex_err)
        exit(mutex_err);
}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table) {
    ...
    int mutex_err = pthread_mutex_destroy(&hash_table->table_mutex);
    if (mutex_err)
        exit(mutex_err);
    free(hash_table);
}
