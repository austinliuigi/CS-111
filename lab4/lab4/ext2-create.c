/*
- (x) write_superblock()
- (x) write_block_group_descriptor_table()
- (x) write_block_bitmap()
- (x) write_inode_bitmap()
- (x) write_inode_table()
- (x) write_root_dir_block()
...
void write_superblock(int fd) {
    ...
    superblock.s_blocks_per_group = BLOCK_SIZE * 8;     /* BLOCK_SIZE bytes per bitmap -> BLOCK_SIZE * 8 bits per bitmap */
    superblock.s_frags_per_group = BLOCK_SIZE * 8;      /* Number of fragments per group */
    superblock.s_inodes_per_group = NUM_INODES;         /* Note: hardcoded for one group */
...
void write_block_group_descriptor_table(int fd) {
    ...
    block_group_descriptor.bg_used_dirs_count = 2; /* hardcoded: root dir & lost and found dir */
...
void write_block_bitmap(int fd)
{
    ...
    map_value[0] = 0b11111111; /* start: block 1 (superblock) */
    map_value[1] = 0b11111111;
    map_value[2] = 0b01111111; /* end: block 23 (hello-world-file-block)  */

    /* pad after max blocks */
    map_value[NUM_BLOCKS/8 - 1] = 0b10000000;
...
void write_inode_bitmap(int fd)
{
    ...
    // TODO It's all yours
    u8 map_value[BLOCK_SIZE] = {0};
    map_value[0] = 0b11111111; /* start: inode 1 (system-used) */
    map_value[1] = 0b00011111; /* end: inode 13 (hello symlink) */
...
void write_inode_table(int fd) {
    ...
    root_inode.i_atime = current_time; /* access time */
    root_inode.i_ctime = current_time; /* creation time */
    root_inode.i_mtime = current_time; /* modification time */
    root_inode.i_dtime = 0; /* deletion time */
    ...
    hello_inode.i_blocks = 0; /* This is a fast symlink that uses no blocks */
    strcpy(hello_inode.i_block, "hello-world"); /* fast symbolic link (target str stored directly in blocks array) */
...
void write_hello_world_file_block(int fd)
{
    ...
    printf("writing hello world contents");
